...
check_release_preflight() {
  ...
  if ! grep -qE '^[[:space:]]+types:[[:space:]]*\\[[[:space:]]*released[[:space:]]*\\][[:space:]]*  grep -qE '^      id-token:[[:space:]]+write$' "$release" || { finding release_preflight "release job lacks id-token: write for attestation"; clean=0; }
...
exit 1 "$release" || { finding release_preflight "release job lacks id-token: write for attestation"; clean=0; }
  grep -qE '^      attestations:[[:space:]]+write
...
if [ "$FINDINGS" -eq 0 ]; then echo "hardening-audit: $RAN check(s) ran, no findings."; exit 0; fi
echo "hardening-audit: $RAN check(s) ran, $FINDINGS finding(s)."
exit 1 "$release" || { finding release_preflight "release job lacks attestations: write"; clean=0; }
  grep -qE 'uses:[[:space:]]+actions/attest@[0-9a-f]{40}' "$release" || { finding release_preflight "attestation action is not full-SHA pinned"; clean=0; }
  grep -qF './gradlew verifyPluginSignature -PbuildVersion="$RELEASE_VERSION"' "$release" || { finding release_preflight "signature verification is missing before publication"; clean=0; }
  grep -qE 'artifacts=\(\.\/build\/distributions\/\*-"\$RELEASE_VERSION"-signed\.zip\)' "$release" || { finding release_preflight "signed release artifact selection is missing or ambiguous"; clean=0; }
  grep -qF 'RELEASE_ARTIFACT_PATH: ${{ steps.signed_artifact.outputs.path }}' "$release" || { finding release_preflight "GitHub Release upload is not bound to the verified signed artifact"; clean=0; }
...
echo "hardening-audit: $RAN check(s) ran, $FINDINGS finding(s)."
exit 1 "$release"; then
    finding release_preflight "release publication trigger must be stable released only"; clean=0
  fi
  if [ "$(unquote "$(job_field "$release" release environment)")" != "jetbrains-marketplace" ]; then
    finding release_preflight "release job is not bound to jetbrains-marketplace environment"; clean=0
  fi
