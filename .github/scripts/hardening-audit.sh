...
check_release_preflight() {
  ...
  grep -qF 'types: [ released ]' "$release" || { finding release_preflight "release workflow must publish only from stable released events"; clean=0; }
  ! grep -qF 'prereleased' "$release" || { finding release_preflight "release workflow must not auto-publish GitHub prereleases to the default Marketplace channel"; clean=0; }
  grep -qE '^    environment:[[:space:]]+jetbrains-marketplace$' "$release" || { finding release_preflight "release job is not bound to jetbrains-marketplace environment"; clean=0; }
  ...
  grep -qF 'Published identity requires its original pending identity.' "$release" || { finding release_preflight "published release state is not anchored to the original pending identity"; clean=0; }
  grep -qF '[ "$pending_canonical" = "$published_canonical" ]' "$release" || { finding release_preflight "pending/published release identity equality check is missing"; clean=0; }
  ...
  upload_pos="$(grep -nF 'gh release upload "$RELEASE_TAG" "$RELEASE_ARTIFACT_PATH"' "$release" | head -1 | cut -d: -f1 || true)"
