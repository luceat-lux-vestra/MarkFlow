...
check_policy_shape() {
  ...
  if ! jq -e '
    (.security_settings | type == "object") and
    (.security_settings.dependency_graph | type == "boolean") and
    (.security_settings.dependabot_alerts | type == "boolean") and
    (.security_settings.dependabot_security_updates | type == "string") and
    (.security_settings.secret_scanning | type == "string") and
    (.security_settings.secret_scanning_push_protection | type == "string") and
    (.security_settings.private_vulnerability_reporting | type == "boolean") and
    (.security_settings.codeql_default_setup_state | type == "string") and
    (.actions_settings.default_workflow_permissions | type == "string") and
    (.actions_settings.can_approve_pull_request_reviews | type == "boolean")
...
check_repository_security() {
  local repo_detail pvr codeql actions expected clean=1

  if [ "$NETWORK" -eq 0 ]; then info repository_security "skipped (--no-network)"; return 0; fi
  if ! command -v gh >/dev/null 2>&1 || [ -z "${GITHUB_REPOSITORY:-}" ]; then
    finding repository_security "gh and GITHUB_REPOSITORY are required for live security readback"; return 0
  fi
  if [ -z "${HARDENING_AUDIT_TOKEN:-}" ]; then
    finding repository_security "authoritative HARDENING_AUDIT_TOKEN is unavailable; public repository security state is unverified"; return 0
  fi

  if jq -e '.security_settings.dependency_graph == true' "$POLICY" >/dev/null; then
    if ! authoritative_gh api "repos/${GITHUB_REPOSITORY}/dependency-graph/sbom" >/dev/null 2>&1; then
      finding repository_security "Dependency Graph/SBOM readback failed"; clean=0
    fi
  fi

  if jq -e '.security_settings.dependabot_alerts == true' "$POLICY" >/dev/null; then
    if ! authoritative_gh api "repos/${GITHUB_REPOSITORY}/vulnerability-alerts" >/dev/null 2>&1; then
      finding repository_security "Dependabot alerts readback failed or alerts are disabled"; clean=0
    fi
  fi

  if ! repo_detail="$(authoritative_gh api "repos/${GITHUB_REPOSITORY}" 2>/dev/null)"; then
    finding repository_security "could not read repository security_and_analysis state"; return 0
  fi
  if ! jq -e '.security_and_analysis | type == "object"' <<< "$repo_detail" >/dev/null; then
    finding repository_security "repository response omitted security_and_analysis; refusing to infer security state"; return 0
  fi

  for field in dependabot_security_updates secret_scanning secret_scanning_push_protection; do
    expected="$(jq -r --arg field "$field" '.security_settings[$field]' "$POLICY")"
    if ! jq -e --arg field "$field" --arg expected "$expected"       '.security_and_analysis[$field].status == $expected' <<< "$repo_detail" >/dev/null; then
      finding repository_security "$field does not match canonical expected state '$expected'"; clean=0
    fi
  done

  if ! pvr="$(authoritative_gh api "repos/${GITHUB_REPOSITORY}/private-vulnerability-reporting" 2>/dev/null)"; then
    finding repository_security "private vulnerability reporting readback failed"; clean=0
  elif ! jq -e --argjson expected "$(jq '.security_settings.private_vulnerability_reporting' "$POLICY")"       '.enabled == $expected' <<< "$pvr" >/dev/null; then
    finding repository_security "private vulnerability reporting does not match canonical policy"; clean=0
  fi

  if ! codeql="$(authoritative_gh api "repos/${GITHUB_REPOSITORY}/code-scanning/default-setup" 2>/dev/null)"; then
    finding repository_security "CodeQL default-setup readback failed"; clean=0
  else
    expected="$(jq -r '.security_settings.codeql_default_setup_state' "$POLICY")"
    if ! jq -e --arg expected "$expected" '.state == $expected' <<< "$codeql" >/dev/null; then
      finding repository_security "CodeQL default setup state does not match '$expected'; custom and default authority must not drift silently"; clean=0
    fi
  fi

  if ! actions="$(authoritative_gh api "repos/${GITHUB_REPOSITORY}/actions/permissions/workflow" 2>/dev/null)"; then
    finding repository_security "Actions default workflow-permission readback failed"; clean=0
  else
    expected="$(jq -r '.actions_settings.default_workflow_permissions' "$POLICY")"
    if ! jq -e --arg expected "$expected" '.default_workflow_permissions == $expected' <<< "$actions" >/dev/null; then
      finding repository_security "default_workflow_permissions drifted from '$expected'"; clean=0
    fi
    if ! jq -e --argjson expected "$(jq '.actions_settings.can_approve_pull_request_reviews' "$POLICY")"       '.can_approve_pull_request_reviews == $expected' <<< "$actions" >/dev/null; then
      finding repository_security "Actions PR-review approval permission drifted"; clean=0
    fi
  fi

  [ "$clean" -eq 1 ] && info repository_security "authoritative public security and Actions settings match canonical policy"
  return 0
}
...
ALL_CHECKS="policy_shape policy_producers negative_tests action_pinning workflow_permissions workflow_security privileged_workflows live_readback_boundary workflow_static_analysis release_preflight ruleset_sync release_tag_ruleset label_references repository_settings repository_security"
should_run() {
...
for check in $ALL_CHECKS; do
  ...
  case "$check" in
    ...
    repository_security) check_repository_security ;;
...
exit 1
\t' read -r context workflow job; do
    [ -n "$context" ] || continue
    wf="$ROOT/$workflow"
    if [ ! -f "$wf" ]; then finding policy_shape "staged context '$context' names missing workflow '$workflow'"; clean=0; continue; fi
    if ! job_exists "$wf" "$job"; then finding policy_shape "staged context '$context' names missing job '$job'"; clean=0; continue; fi
