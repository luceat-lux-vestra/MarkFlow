...
check_release_authority() {
  local root="$1" release="$1/.github/workflows/release.yml"
...
check_release_authority "$baseline"
...
workflow_scope_write="$TMP/workflow-scope-write"
copy_root "$workflow_scope_write"
...
expect_release_authority_fail "$prerelease_trigger" "release publication trigger must be stable released only"
...
expect_release_authority_fail "$missing_marketplace_environment" "release job is not bound to jetbrains-marketplace environment"
...
echo "hardening-audit negative fixtures passed" "$release" || {
...
expect_release_authority_fail() {
  local root="$1" needle="$2" output status=0
  output="$(check_release_authority "$root" 2>&1)" || status=$?
  [ "$status" -ne 0 ] || { echo "release authority fixture unexpectedly passed" >&2; return 1; }
  case "$output" in *"$needle"*) ;; *) echo "release authority fixture failed for the wrong reason: $needle" >&2; printf '%s\n' "$output" >&2; return 1 ;; esac
}
