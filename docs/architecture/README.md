...
6. **Mermaid and KaTeX remain supported renderer capabilities.** The retained production engines are Mermaid `12.0.0` and KaTeX `^0.18.7`; editor adapters are replaceable and renderer upgrades remain separately gated. Mermaid 12 compatibility is intentional: MarkFlow explicitly preserves classic look and Dagre layout instead of accepting the new upstream defaults implicitly.
